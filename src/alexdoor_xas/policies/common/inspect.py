...
Stitches deterministic chunk predictions along each episode's recorded
observations (no environment involved) and reports per-dimension L1/MSE — the
cheap sanity check between offline loss and closed-loop rollout. The policy is
duck-typed: it needs ``predict(obs) -> (H, D)``, ``obs_preset``,
``action_space``, ``chunk_size``, and ``stats``. Matplotlib is lazy and
...
def predict_episode_open_loop(
    ...
    """Predicted actions ``(N, D)``: chunks queried at t = 0, stride, 2·stride, ...

    ``stride`` defaults to the policy's full chunk size; a receding-horizon
    policy passes its execution horizon (Ta) so inspection matches rollout.
    """
...
def _open_loop_plot(record: EpisodeRecord, predicted: np.ndarray, path: Path) -> Path:
    ...
    for dim, (ax, label) in enumerate(zip(axes, labels, strict=True)):
        ...
        ax.set_ylabel(label)
    axes[0].set_title(
        f"Open-loop prediction — episode {record.episode_id[:8]} "
