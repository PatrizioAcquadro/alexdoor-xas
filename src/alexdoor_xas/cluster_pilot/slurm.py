    ...
    directives.extend(
        [
            ...
            *_render_cells(config.cells),
            ...
            '      for name in environment slurm status; do',
            '        if [[ -d "$CELL_RUNTIME/$name" ]] && ! cp -a "$CELL_RUNTIME/$name" "$publish_tmp/$name"; then publish_ok=0; break; fi',
            "      done",
            "    fi",
            '    if [[ $publish_ok -eq 1 && -d "$CELL_RUNTIME/wandb" ]]; then',
            '      if ! "$CONDA_PREFIX/bin/python" src/alexdoor_xas/cluster_pilot/wandb_publication.py \\',
            '        --source "$CELL_RUNTIME/wandb" --destination "$publish_tmp/wandb"; then publish_ok=0; fi',
            "    else",
            "      publish_ok=0",
            "    fi",
