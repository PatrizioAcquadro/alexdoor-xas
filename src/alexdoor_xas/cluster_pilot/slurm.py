...
def render_slurm_script(
    ...
    *,
    ...
    if config.slurm.gpus_per_task != 1:
        raise SlurmRenderError("the pilot must request exactly one GPU per task")
    ...
    directives = [
        ...
        f"#SBATCH --gres=gpu:{config.slurm.gpus_per_task}",
    ...
    directives.extend(
        [
            ...
            'RUN_OUTPUT_ROOT="$SCRATCH_RUNS_ROOT/runs"',
            "",
            ...
            'mkdir -p "$SCRATCH_RUNS_ROOT" "$RUN_OUTPUT_ROOT" "$DURABLE_RESULTS_ROOT"',
            'cd "$REPO_ROOT"',
            ...
            *_render_cells(config.cells),
            ...
            "esac",
            "",
            'CELL_RUNTIME="$SCRATCH_RUNS_ROOT/cells/$RUN_ID"',
            ...
            "  printf '{\"schema\":\"alexdoor_xas.cluster_pilot_cell_status.v1\",\"status\":\"%s\",\"run_id\":\"%s\",\"policy\":\"%s\",\"space\":\"%s\",\"exit_code\":%d,\"source_git_commit\":\"%s\"}\\n' \\",
            '    "$status_value" "$RUN_ID" "$POLICY" "$SPACE" "$code" "$SOURCE_COMMIT" > "$status_tmp"',
            ...
            '  local job_key="${SLURM_ARRAY_JOB_ID:-${SLURM_JOB_ID:-manual}}.${TASK_ID}"',
            '  local publish_tmp="$DURABLE_RESULTS_ROOT/.${RUN_ID}.${job_key}.tmp"',
            '  local publish_final="$DURABLE_RESULTS_ROOT/$RUN_ID"',
            '  if [[ $status_ok -eq 0 ]]; then',
            '    echo "could not publish atomic cell status" >&2',
            '  elif [[ -e "$publish_tmp" || -e "$publish_final" ]]; then',
            ...
            "      done",
            "    fi",
            '    if [[ $publish_ok -eq 1 ]] && ! mv "$publish_tmp" "$publish_final"; then publish_ok=0; fi',
